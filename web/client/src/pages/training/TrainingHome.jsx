...
export default function TrainingHome() {
  ...
  const cpo = ladder.find((c) => c.key === 'CPO');
  const cst = ladder.find((c) => c.key === 'CST');
  const specialties = ladder.filter((c) => c.key === 'RESIDENTIAL' || c.key === 'COMMERCIAL');
    ...
    <div className="page">
      <div className="page-header">
        ...
        <p className="text-secondary" style={{ marginTop: '0.25rem' }}>
          Pool & spa certification paths
        </p>
      </div>

      <div className="stack">
        {/* CPO */}
        {cpo && <CertCard cert={cpo} onNavigate={goTo} />}
        ...
        {/* CST */}
        {cst && <CertCard cert={cst} onNavigate={goTo} />}
        ...
        {specialties.length > 0 && (
          <>
            ...
            <div>
              <h3 style={{ textAlign: 'center', marginBottom: '0.75rem', color: '#A0A0A8', fontSize: '0.875rem' }}>
                Specialties (choose track)
              </h3>
              <div style={{
                display: 'grid',
                ...
                {specialties.map((cert) => (
                  <CertCard key={cert.key} cert={cert} onNavigate={goTo} />
                ))}
              </div>
            </div>
          </>
